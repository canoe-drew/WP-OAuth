...
$_SESSION['WPOA']['PROVIDER'] = 'idQ';
define('HTTP_UTIL', get_option('wpoa_http_util'));
define('CLIENT_ENABLED', get_option('wpoa_idq_api_enabled'));
define('CLIENT_ID', get_option('wpoa_idq_api_id'));
define('CLIENT_SECRET', get_option('wpoa_idq_api_secret'));
define('REDIRECT_URI', rtrim(site_url(), '/') . '/');
define('SCOPE', 'profile'); // PROVIDER SPECIFIC: 'profile' is the minimum scope required to get the user's id from Google
define('URL_AUTH', "https://taas.idquanta.com/idqoauth/api/v1/auth?");
define('URL_TOKEN', "https://taas.idquanta.com/idqoauth/api/v1/token?");
define('URL_USER', "https://taas.idquanta.com/idqoauth/api/v1/user?");
define('IDENTITY_KEY','username');
# END OF DEFINE THE OAUTH PROVIDER AND SETTINGS TO USE #

#
# Below this point, this file is identical to login-google except
# for the key to to access the oauth identity from the result object.
#
...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['id'] = $result_obj[IDENTITY_KEY]; // PROVIDER SPECIFIC: idQ returns the user's OAuth identity as 'username'
